...
const GeneratorPage = ({ user, setPage }) => {
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            setError(`Terjadi kesalahan: ${error.message || 'Error tidak diketahui'}`);
            console.error(e);
        } finally {
            setIsLoading(false);
        }
    ...
    const handleAnalysis = async () => {
        ...
        try {
            ...
            const error = e as { message?: string };
            setAnalysisResult(`Gagal menganalisis: ${error.message || 'Error tidak diketahui'}`);
            console.error(e);
        } finally {
            ...
            <div className="grid lg:grid-cols-2 gap-8">
                ...
                <div className="bg-white p-6 rounded-lg shadow-md">
                    <div className="space-y-4">
                        ...
                        <button onClick={handleGenerate} disabled={isLoading} className="w-full bg-indigo-600 text-white font-bold py-3 rounded-lg hover:bg-indigo-700 disabled:bg-indigo-300 transition-colors">
                            {isLoading ? 'Membuat Skrip...' : '✨ Generate Script'}
                        </button>
                ...
                <div className="bg-gray-50 p-6 rounded-lg shadow-inner">
                    <h2 className="text-xl font-bold mb-4">Hasil Skrip Anda</h2>
                    {isLoading && <div className="text-center py-10"><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-indigo-600 mx-auto"></div><p className="mt-4">AI sedang berpikir...</p></div>}
                    {!isLoading && !generatedScript && <div className="text-center text-gray-500 py-10"><p>Hasil skrip akan muncul di sini setelah Anda menekan tombol "Generate Script".</p></div>}
                    {generatedScript && (
                        <div className="whitespace-pre-wrap bg-white p-4 rounded-md shadow-sm">
                    ...
                    {generatedHashtags && (
                        <div className="mt-6">
                            <h3 className="text-lg font-bold mb-2">Template Hashtag</h3>
                             <div className="bg-white p-4 rounded-md shadow-sm text-gray-700">
                                {generatedHashtags}
                            </div>
                        </div>
                    )}
                    {generatedScript && (
                        <div className="mt-6 flex flex-col gap-3">
                             <button onClick={handleAnalysis} disabled={isAnalyzing} className="w-full bg-purple-600 text-white font-bold py-2 px-4 rounded-lg hover:bg-purple-700 transition-colors disabled:bg-purple-300">
                                <SparklesIcon />
                                {isAnalyzing ? 'Menganalisis...' : 'Analisis & Ide Hook'}
                            </button>
                            <div className="flex flex-col sm:flex-row gap-3">
                                <button onClick={() => copyToClipboard(generatedScript + "\n\n" + generatedHashtags)} className="flex-1 bg-gray-600 text-white font-bold py-2 px-4 rounded-lg hover:bg-gray-700 transition-colors">Salin Skrip & Hashtag</button>
                                {user?.role === 'pro' ? (
                                    <button onClick={downloadScript} className="flex-1 bg-green-600 text-white font-bold py-2 px-4 rounded-lg hover:bg-green-700 transition-colors">Unduh (.txt)</button>
                                ) : (
                                    <button onClick={() => setPage('pricing')} className="flex-1 bg-yellow-500 text-white font-bold py-2 px-4 rounded-lg hover:bg-yellow-600 transition-colors"><LockIcon /> Upgrade untuk Unduh</button>
                                )}
                            </div>
                        </div>
                    )}
                    {user?.role !== 'pro' && !isLoading && (
                        <div className="mt-8 p-4 bg-gray-200 rounded-lg text-center text-gray-600">
                            <p className="font-bold">Area Iklan</p>
                            <p className="text-sm">Upgrade ke Pro untuk pengalaman bebas iklan.</p>
                        </div>
                    )}
                </div>
            </div>
...
const PricingPage = ({ user, setPage }) => {
    const handleUpgrade = () => {
        ...
        // Di sini logika integrasi Midtrans/Stripe akan dipanggil
        alert("Fitur pembayaran sedang dalam pengembangan. Anda akan diarahkan ke halaman pembayaran.");
    };

    const Feature = ({ children, isPro }) => (
        <li className="flex items-center">
            {isPro ? <CheckCircleIcon /> : <XCircleIcon />}
            <span>{children}</span>
        </li>
    );
    ...
    return (
        <div className="container mx-auto p-4 md:p-8">
            <h1 className="text-4xl font-extrabold text-center text-gray-800 mb-4">Paket Kami</h1>
            <p className="text-center text-gray-600 max-w-xl mx-auto mb-12">Pilih paket yang paling sesuai dengan kebutuhan Anda untuk menjadi super affiliate.</p>
            <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
                {/* Paket Gratis */}
                <div className="border rounded-lg p-8">
                    <h2 className="text-2xl font-bold">Gratis</h2>
                    <p className="text-gray-500">Untuk memulai dan mencoba</p>
                    <p className="text-4xl font-bold my-4">Rp 0</p>
                    <ul className="space-y-3">
                        <Feature isPro={true}>5 Generate Harian</Feature>
                        <Feature isPro={true}>Gaya Video Dasar</Feature>
                        <Feature isPro={false}>Download Skrip</Feature>
                        <Feature isPro={false}>Template Hashtag</Feature>
                        <Feature isPro={false}>Riwayat Generator</Feature>
                        <Feature isPro={false}>Tanpa Iklan</Feature>
                    </ul>
                </div>
                {/* Paket Pro */}
                <div className="border-2 border-indigo-600 rounded-lg p-8 relative">
                    <span className="absolute top-0 -translate-y-1/2 bg-indigo-600 text-white text-sm font-bold px-3 py-1 rounded-full">Paling Populer</span>
                    <h2 className="text-2xl font-bold text-indigo-600">Pro</h2>
                    <p className="text-gray-500">Untuk kreator dan afiliator serius</p>
                    <p className="text-4xl font-bold my-4">Rp 99.000<span className="text-lg font-normal">/bulan</span></p>
                     <ul className="space-y-3">
                        <Feature isPro={true}>Generate Tanpa Batas</Feature>
                        <Feature isPro={true}>Semua Gaya Video (Termasuk Gaul, Persuasif)</Feature>
                        <Feature isPro={true}>Download Skrip (.txt)</Feature>
                        <Feature isPro={true}>Template Hashtag Cerdas</Feature>
                        <Feature isPro={true}>Riwayat Generator</Feature>
                        <Feature isPro={true}>Bebas Iklan</Feature>
                    </ul>
                    <button onClick={handleUpgrade} className="w-full mt-8 bg-indigo-600 text-white font-bold py-3 rounded-lg hover:bg-indigo-700 transition-colors">
                        Upgrade ke Pro
                    </button>
                </div>
            </div>
        ...
        if (user?.role === 'pro') {
            const fetchHistory = async () => {
                try {
                    ...
                    const historyData = querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })) as ScriptHistory[];
                    historyData.sort((a, b) => b.createdAt.toDate().getTime() - a.createdAt.toDate().getTime());
    ...
    return (
        <div className="container mx-auto p-4 md:p-8">
            ...
            <div className="bg-white p-6 rounded-lg shadow-md mb-8">
                <h2 className="text-xl font-bold mb-2">Informasi Akun</h2>
                <p><strong>Email:</strong> {user.email}</p>
                <p><strong>Status Langganan:</strong> <span className={`font-bold ${user.role === 'pro' ? 'text-indigo-600' : (user.role === 'admin' ? 'text-red-600' : 'text-gray-600')}`}>{user.role.toUpperCase()}</span></p>
            </div>

            {user.role === 'pro' && (
                <div className="bg-white p-6 rounded-lg shadow-md">
                    <h2 className="text-xl font-bold mb-4">Riwayat Skrip Anda</h2>
                    {isLoading && <p>Memuat riwayat...</p>}
                    {!isLoading && history.length === 0 && <p>Anda belum memiliki riwayat skrip.</p>}
                    {!isLoading && history.length > 0 && (
                        <div className="space-y-4">
                            {history.map(item => (
                                <div key={item.id} className="border p-4 rounded-md">
                                    <p className="font-bold">{item.productName}</p>
                                    <p className="text-sm text-gray-500">Gaya: {item.videoStyle} | Dibuat pada: {item.createdAt.toDate().toLocaleDateString()}</p>
                                    <details className="mt-2">
                                        <summary className="cursor-pointer text-indigo-600">Lihat Skrip</summary>
                                        <pre className="whitespace-pre-wrap bg-gray-50 p-2 mt-2 rounded">{item.generatedScript}</pre>
                                        {item.generatedHashtags && <pre className="whitespace-pre-wrap bg-gray-50 p-2 mt-2 rounded">{item.generatedHashtags}</pre>}
                                    </details>
                                </div>
                            ))}
                        </div>
...
const LoginPage = ({ setPage }) => {
    ...
    const handleAuth = async (e) => {
        ...
        try {
            ...
            setError(firebaseError.message || 'Terjadi kesalahan');
        }
    };
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-gray-50">
            <div className="max-w-md w-full bg-white p-8 rounded-lg shadow-lg">
                ...
                <button onClick={handleGoogleLogin} className="w-full bg-white border border-gray-300 text-gray-700 font-bold py-2 rounded-lg hover:bg-gray-100 transition-colors flex items-center justify-center">
                ...
                <p className="text-center mt-6">
                    ...
                    <button onClick={() => setIsLogin(!isLogin)} className="text-indigo-600 font-bold ml-2">
...
const AdminPanelPage = () => {
    ...
    useEffect(() => {
        const loadSettings = async () => {
            ...
            setMidtransKey(settings.midtransKey || '');
            setIsLoading(false);
        };
        loadSettings();
    }, []);
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const saveError = error as { message?: string };
            setMessage(`Gagal menyimpan: ${saveError.message || 'Error tidak diketahui'}`);
            console.error(error);
        }
    };

    if (isLoading) {
        return <div className="text-center p-10">Memuat pengaturan...</div>;
    }
        ...
        <div className="container mx-auto p-4 md:p-8">
            ...
            <div className="max-w-2xl mx-auto bg-white p-6 rounded-lg shadow-md">
                <div className="space-y-6">
                    <div>
                        <label className="font-bold text-gray-700">Gemini API Key</label>
                        <p className="text-sm text-gray-500 mb-2">Kunci API untuk menghubungkan ke layanan AI Gemini.</p>
                        <input 
                            type="password" 
                            value={geminiApiKey} 
                    ...
                    <div>
                        <label className="font-bold text-gray-700">Midtrans Client Key</label>
                         <p className="text-sm text-gray-500 mb-2">Kunci untuk integrasi pembayaran Midtrans (fitur masa depan).</p>
                        <input 
                            type="password" 
                            value={midtransKey} 
                            onChange={e => setMidtransKey(e.target.value)} 
                            className="w-full p-2 border rounded"
                            placeholder="Masukkan kunci client Midtrans Anda"
                        />
                    </div>
                    <button 
                        onClick={handleSave} 
                    ...
                    {message && <p className={`text-sm mt-4 p-3 rounded ${message.includes('Gagal') ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'}`}>{message}</p>}
                </div>
            </div>
        </div>
    );
...
export default function App() {
    ...
    const renderPage = () => {
        switch (page) {
            case 'generator':
                return <GeneratorPage user={user} setPage={setPage} />;
            case 'pricing':
                return <PricingPage user={user} setPage={setPage} />;
            case 'dashboard':
                return user ? <DashboardPage user={user} /> : <LoginPage setPage={setPage} />;
            case 'login':
                return <LoginPage setPage={setPage} />;
            case 'admin':
                return user?.role === 'admin' ? <AdminPanelPage /> : <HomePage setPage={setPage} />;
            default:
    ...
    return (
        <div className="bg-gray-100 min-h-screen font-sans">
            <nav className="bg-white shadow-sm sticky top-0 z-50">
                <div className="container mx-auto px-4 py-3 flex justify-between items-center">
                    ...
                    <div className="hidden md:flex items-center space-x-6">
                        <button onClick={() => setPage('home')} className="text-gray-600 hover:text-indigo-600">Home</button>
                        <button onClick={() => setPage('generator')} className="text-gray-600 hover:text-indigo-600">Generator</button>
                        <button onClick={() => setPage('pricing')} className="text-gray-600 hover:text-indigo-600">Paket Pro</button>
                        {user && <button onClick={() => setPage('dashboard')} className="text-gray-600 hover:text-indigo-600">Dashboard</button>}
                        {user?.role === 'admin' && <button onClick={() => setPage('admin')} className="text-red-600 font-bold hover:text-red-800">Admin Panel</button>}
                    </div>
                    <div>
                        {user ? (
                            ...
                            <button onClick={() => setPage('login')} className="bg-indigo-600 text-white font-bold py-2 px-4 rounded-lg hover:bg-indigo-700 transition-colors">Login</button>
                        )}
                    </div>
                </div>
            </nav>
            ...
            <footer className="bg-gray-800 text-white text-center p-6 mt-12">
                <p>&copy; 2025 ScriptGen. Dibuat untuk para Affiliate Juara.</p>
            </footer>
        </div>
    );
}
